...
def MaskingLogits(inp):
    ...
    unnormProbs = tf.exp(
        logitsRep)  # constructing the logistic normal distribution, by the way transform all number to positive
...
def real2complex(x):  # 双通道转为复数，tf操作
    return tf.stack([tf.complex(x[:, :, :, 0], x[:, :, :, 1]), tf.complex(x[:, :, :, 2], x[:, :, :, 3])], axis=-1)


def fftshift(x):
    s0 = (x.shape[-2] // 2)  # 32
    s1 = (x.shape[-1] // 2)  # 32
    x = tf.concat([x[:, :, s0:, :], x[:, :, :s0, :]], axis=-2)  # exchange down and up
    x = tf.concat([x[:, :, :, s1:], x[:, :, :, :s1]], axis=-1)  # exchange right and left
    return x
...
def dc(inp):
    ...
    gene_complex = ifftshift(gene_complex)
    gene_ifft = tf.ifft2d(gene_complex)
    ...
    output_complex = fftshift(output_complex)
    output_complex = tf.transpose(output_complex, [0, 2, 3, 1])
    return complex_real(output_complex)
...
def complex_real(x):  # 复数转为双通道，tf操作
    output1_real = tf.cast(tf.real(x[:, :, :, 0]), dtype=tf.float32)
    output1_imag = tf.cast(tf.imag(x[:, :, :, 0]), dtype=tf.float32)
    output2_real = tf.cast(tf.real(x[:, :, :, 1]), dtype=tf.float32)
    output2_imag = tf.cast(tf.imag(x[:, :, :, 1]), dtype=tf.float32)
    output = tf.stack([output1_real, output1_imag, output2_real, output2_imag], axis=-1)
    return output
...
def full_model(input_dim, DPS_dim, mux_out, tempIncr, entropyMult, n_epochs, batchPerEpoch):
    ...
    print('samples shape:', Amatrix.shape)
    upSampledInp = Lambda(Ax, name="HardSampling")([input_layer, Amatrix])  # Amatrix is the sampling matrix
    print('Shape after inverse A: ', upSampledInp.shape)  # upSampleInp is the undersampling data
    ...
    def HRU0_layer(x, upSampledInp, Amatrix, concat_tensor, stage, block):
        ...
        conv0 = Conv2DTranspose(filters=4, kernel_size=(n1, n2), strides=(1, 1), padding="same",
    ...
    def HRU_layer(x, upSampledInp, Amatrix, concat_tensor, de1, de2, de3, stage, block):
        ...
        conv0 = Conv2D(filters=4, kernel_size=(n1, n2), strides=(1, 1), padding="same",
                       name='out' + conv_name_base)(up1)
        conv0 = BatchNormalization(gamma_initializer='glorot_normal', axis=3, name='out' + bn_name_base)(conv0)
        conv0 = LeakyReLU(alpha=0.2, name='out' + relu_name_base)(conv0)
    ...
    temp_3, de1_s2, de2_s2, de3_s2 = HRU_layer(temp_2, upSampledInp, Amatrix, temp_2, de1_s1, de2_s1, de3_s1,
                                               stage=False, block='SDN2')
    temp_31 = concatenate([temp_2, temp_3], name='concat_2')
    temp_4, de1_s3, de2_s3, de3_s3 = HRU_layer(temp_3, upSampledInp, Amatrix, temp_31, de1_s2, de2_s2, de3_s2,
                                               stage=False, block='SDN3')
    temp_41 = concatenate([temp_2, temp_3, temp_4], name='concat_3')
    temp_5, de1_s4, de2_s4, de3_s4 = HRU_layer(temp_4, upSampledInp, Amatrix, temp_41, de1_s3, de2_s3, de3_s3,
                                               stage=False, block='SDN4')
    temp_51 = concatenate([temp_2, temp_3, temp_4, temp_5], name='concat_4')
    temp_6, de1_s5, de2_s5, de3_s5 = HRU_layer(temp_5, upSampledInp, Amatrix, temp_51, de1_s4, de2_s4, de3_s4,
                                               stage=False, block='SDN5')
    temp_61 = concatenate([temp_2, temp_3, temp_4, temp_5, temp_6], name='concat_5')
    temp_7, de1_s6, de2_s6, de3_s6 = HRU_layer(temp_6, upSampledInp, Amatrix, temp_61, de1_s5, de2_s5, de3_s5,
                                               stage=False, block='SDN6')
...
def recModel(fid_rows, fid_cols):
    upSampledInp = Input(shape=[fid_rows, fid_cols, 4], name="HardSampling")
